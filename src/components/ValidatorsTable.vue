    ...
    <div class="flex flex-col flex-1 space-y-4">
        ...
        <div class="flex flex-none space-x-4">
            <button
                ...
                @click="validatorsStore.setShowElected(!showElected)"
            >Show elected validators ({{ amountOfElected(validators) }})</button>
            <button
                ...
                class="px-3 py-2 rounded-lg text-sm"
                @click="validatorsStore.setShowNonElected(!showNonElected)"
            >Show not elected validators ({{ amountOfNonElected(validators) }})</button>
        </div>
        <div class="overflow-x-auto">
            <div class="inline-block min-w-full">
                <div
                    class="overflow-hidden shadow-md border border-slightGray dark:border-slightDark rounded-lg"
                >
                    <table class="min-w-full">
                        <thead
                            class="from-slightGray dark:from-oswapDark-gray dark:to-slightDark bg-gradient-to-r"
                        >
                            <tr
                            ...
                            >
                                <th scope="col" class="py-3 px-6 table-cell">
                                    <div
                                        class="flex flex-none items-center cursor-pointer select-none opacity-100 hover:opacity-80"
                                    ...
                                    >
                                        ...
                                        <template v-if="order === SORT_FIELD.STATUS">
                                            <i
                                                class="las la-arrow-down text-oswapGreen-light pl-1 dark:text-oswapGreen"
                                                v-if="!direction"
                                            ></i>
                                            <i
                                                class="las la-arrow-up text-oswapGreen-light pl-1 dark:text-oswapGreen"
                                                v-if="direction"
                                            ></i>
                                        </template>
                                    </div>
                                ...
                                <th scope="col" class="lg:py-4 lg:px-6 md:py-3 md:px-6 py-1.5 px-1">
                                    <div
                                        class="flex flex-none items-center cursor-pointer justify-center select-none opacity-100 hover:opacity-80"
                        ...
                        <tbody>
                            ...
                            <tr
                                ...
                                @click="redirectDetail(validator.address)"
                                v-for="(validator, index) in getPagination"
                                :key="validator.address"
                            >
                                <td
                                    class="lg:py-4 lg:px-6 md:py-4 md:px-6 py-2 px-2 text-xs md:text-sm lg:text-sm"
                                >
                                    <div
                                        class="flex flex-none items-center justify-center md:justify-center space-x-0"
                                    >
                                ...
                                >
                                    <div
                                        class="flex flex-none space-x-2 items-center justify-center md:justify-start"
                                    >
                                        <div class="hidden md:flex flex-none">
                                            <ValidatorLogo
                                                logoClass="rounded-full p-1 w-12 h-12 "
                                                :pixeles="48"
                                ...
                                <td
                                    :class="parseFloat(validator.apr) > 0.14 ? 'text-red-400 ' : ' text-gray-500 dark:text-gray-300'"
                                ...
                                <td
                                    :class="BigNumber.from(validator.total_stake.toString().split('.')[0]).gt(BigNumber.from(5000000)) ? ' text-gray-500 dark:text-gray-300' : ' text-red-400'"
                                    class="lg:py-4 lg:px-6 md:py-4 md:px-6 py-2 px-1 text-xs md:text-sm lg:text-sm whitespace-nowrap hidden md:table-cell"
                                >{{ returnAmounts(validator.total_stake) }}</td>
                                <td
                                    :class="parseFloat(validator.rate) > 0.15 ? 'text-red-400 ' : 'text-gray-500 dark:text-gray-300 '"
                                    class="lg:py-4 lg:px-6 md:py-4 md:px-6 py-2 px-1 text-xs md:text-sm lg:text-sm whitespace-nowrap hidden md:table-cell"
                                >{{ returnPercentage(validator.rate) }}</td>
                                <td
                                    :class="parseFloat(validator.uptime_percentage) < 0.90 ? 'text-red-400 ' : 'text-gray-500 dark:text-gray-300 '"
                            ...
                            >
                                ...
                                >
                                    <div class="flex flex-1 justify-center items-center">
                                        <i
                                            @click="validatorsStore.prevPage()"
                                            :class="pageIndex === 0 ? 'opacity-50 cursor-default' : ''"
                                            class="las la-arrow-left mr-2 text-xl text-oswapBlue dark:text-oswapBlue-light cursor-pointer"
                                        ></i>
                                        Page {{ pageIndex + 1 }} of {{ getAmountOfPages }}
                                        <i
                                            :class="pageIndex + 1 === getAmountOfPages ? 'opacity-50 cursor-default' : ''"
                                            @click="validatorsStore.nextPage()"
                                            class="las la-arrow-right ml-2 text-xl text-oswapBlue dark:text-oswapBlue-light cursor-pointer"
                                        ></i>
                                    </div>
                                </td>
                            </tr>
