...
import { Delegation, Undelegation } from '@/utility/delations.interface';
...
function getUndelegationEpoch(undelEpoch: number) {
    return undelEpoch - epoch.value + 8
}
    ...
    >
        <div class="flex flex-1 justify-between items-center">
            ...
            >
                <ValidatorLogo
                    :pixeles="28"
                    :address="props.Args.delegated.validator_info.address"
            ...
            <div v-if="!showProjected">
                <div class="flex flex-none flex-col">
                    <span class="text-oswapBlue-dark">
                        ...
                        >
                            / {{
                                numeral(parseFloat(utils.formatUnits(props.Args.delegated.amount.toString(), 18)) * parseFloat(onePrice)).format('$0[.]0000')
                            }}
                        </span>
                    </span>
                    <div class="flex flex-none space-x-2 items-center">
                        ...
                        <Popper hover>
                            <div
                                class="flex flex-none relative h-6 items-center"
                            ...
                            <template #content>
                                <div class="flex flex-none flex-col px-2 text-sm space-y-1">
                                    ...
                                    <div
                                        class="flex flex-none space-x-2"
                                        v-for="undel of props.Args.delegated.Undelegations"
                                    >
                                        <span>
                                            - {{ numeral(utils.formatUnits(undel.Amount.toString(), 18)).format('0[.]0000') }}
                                            ONEs
                                        </span>
                                        <span>
                    ...
                    <span class="text-oswapGreen-dark">
                        ...
                        >
                            / {{
                                numeral(parseFloat(utils.formatUnits(props.Args.delegated.reward.toString(), 18)) * parseFloat(onePrice)).format('$0[.]0000')
                            }}
                        </span>
                    </span>
                </div>
                <div class="flex flex-1 justify-end space-x-2 text-white mt-2">
                    <button
                        @click="showProjected = true"
                        class="rounded-md bg-slightGray text-slightDark dark:text-white dark:bg-slightDark px-2 py-1"
                    >Projected</button>
                    <button class="rounded-md bg-oswapBlue-light px-2 py-1">Unstake</button>
                    <button class="rounded-md bg-oswapGreen-dark px-2 py-1">Stake</button>
                </div>
            </div>
